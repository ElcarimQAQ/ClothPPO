...
import numpy as np
from cloth_funnels.utils.env_utils import visualize_grasp
import sys
from filelock import FileLock
import os
import seaborn as sns
import pandas as pd
from cloth_funnels.utils.utils import collect_coverage_stats
from tqdm import tqdm
from pprint import pprint
from loguru import logger
from time import strftime

def summarize(path, plot=True):
    time_str = strftime("%m-%d-%H%M")
    logger.add(f"{os.path.dirname(path)}/eval_{time_str}.log", format="{message}", level="INFO")
    stats = collect_coverage_stats(path, int(1e7))
    for key, value in stats.items():
        if all(word not in key for word in ['distribution', 'img',
                                            'min', 'max', '_steps']):
            logger.info(f'\t[{key:<36}]:\t{value:.04f}')
    # Episode lengths
    if 'episode_length/easy/distribution' in stats:
        easy_episode_num = len(stats['episode_length/easy/distribution'])
        easy_episode_lengths = stats['episode_length/easy/distribution']
        logger.info('Easy Episode Lengths:')
        logger.info('\tnum: {:.02f}'.format(easy_episode_num))
        logger.info('\tmean: {:.04f}'.format(
            np.mean(easy_episode_lengths)))
        logger.info('\t25-quantile: {:.04f}'.format(
            np.quantile(easy_episode_lengths, 0.25)))
        logger.info('\tmedian: {:.04f}'.format(
            np.median(easy_episode_lengths)))
        logger.info('\t75-quantile: {:.04f}'.format(
            np.quantile(easy_episode_lengths, 0.75)))
    if 'episode_length/hard/distribution' in stats:
        hard_episode_num = len(stats['episode_length/hard/distribution'])
        hard_episode_lengths = stats['episode_length/hard/distribution']
        logger.info('Hard Episode Lengths:')
        logger.info('\tnum: {:.02f}'.format(hard_episode_num))
        logger.info('\tmean: {:.02f}'.format(
            np.mean(hard_episode_lengths)))
        logger.info('\t25-quantile: {:.02f}'.format(
            np.quantile(hard_episode_lengths, 0.25)))
        logger.info('\tmedian: {:.02f}'.format(
            np.median(hard_episode_lengths)))
        logger.info('\t75-quantile: {:.02f}'.format(
            np.quantile(hard_episode_lengths, 0.75)))
    if plot:
        pathlib.Path(f'{os.path.dirname(path)}/plot').mkdir(parents=True, exist_ok=True)
        df = pd.DataFrame()
        averaged_coverages = []
        # 均线 window
        window = 1
        ...
        plt.grid()
        plt.savefig(f"{os.path.dirname(path)}/plot/best_coverage.png")
        ...
        plt.grid()
        plt.savefig(f"{os.path.dirname(path)}/plot/final_coverage.png")
        ...
        try:
            ...
            plt.grid()
            plt.savefig(f"{os.path.dirname(path)}/plot/delta_coverage.png")
        ...
        try:
            ...
            plt.grid()
            plt.savefig(f"{os.path.dirname(path)}/plot/postaction_coverage.png")
        except Exception as e:
            pass
        ...
        plt.grid()
        plt.savefig(f"{os.path.dirname(path)}/plot/action_primitive.png")
...
if __name__ == "__main__":
    ...
    logger.info(f'Outputing visualizations to {webpage_path}')
