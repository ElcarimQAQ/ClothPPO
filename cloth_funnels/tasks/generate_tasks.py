...
class TaskLoader:
    def __init__(self, hdf5_path: str, 
        ...
        if eval:
            logging.info("[TaskLoader] Loading eval tasks")
            ...
            logging.info("[TaskLoader] Loading train tasks")
            with h5py.File(self.hdf5_path, 'r') as tasks:
    ...
    def get_next_task(self) -> Task:
        with h5py.File(self.hdf5_path, 'r') as tasks:
            ...
            if not self.repeat:
                ...
                logging.info('[TaskLoader] {}/{} : {}'.format(
                    self.curr_task_idx,
                    len(self.keys),
                    key))
                if self.curr_task_idx >= len(self.keys):
                    logging.info('[TaskLoader] Out of tasks')
                    # sleep(20)
                    # exit(0)
                    while True:
                        sleep(5)
...
if __name__ == "__main__":
    ...
    ray.init(local_mode=args.local_mode, log_to_driver=True)
    if args.test_single:
